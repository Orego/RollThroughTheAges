...
public class DicePanel extends JPanel implements TurnObserver {
	 ...
	 * 
	 * @param dieList
	 *            The list of active dice to display.
	 * @throws TooManyDiceException
	 *             No more than seven dice are allowed
	 */
	public DicePanel(Game game) throws TooManyDiceException {
		...
		for (int i = 0; i < 7; i++) {
			if (i < gamestate.getPlayersDice(currPlayer).size())
				graphicDieList.add(new GraphicDie(gamestate
						.getPlayersDice(currPlayer).get(i).getTopFace(), true));
			else
				graphicDieList.add(new GraphicDie((int) (Math.random() * 6),
						false));
	...
	public void updateGraphicDice() {
		...
		for (int i = 0; i < 7; i++) {
			if (i < dice.size()) {
				...
				graphicDieList.get(i).setEnabled(false);
			}
		}
	}
	...
	public List<Integer> getSelectedIndices() {
		...
		for (int i = 0; i < gamestate.getPlayersDice(
				gamestate.getCurrentPlayer()).size(); i++) {
	...
	/**
	 * The exception that is thrown when too many active dice are passed at
	 * creation.
	 */
	...
	private class RerollListener implements ActionListener {
		...
		@Override
		public void actionPerformed(ActionEvent arg0) {
			gamestate.rerollPlayersDice(gamestate.getCurrentPlayer(),
					getSelectedIndices());
			rollCount.setText("Rerolls left: "
					+ gamestate.getPlayersNumRerolls(gamestate
							.getCurrentPlayer()));
	...
	public void doNewTurnThings() {
		...
		// rollCount
		// .setText("Rerolls left: "+gamestate.getPlayersNumRerolls(gamestate.getCurrentPlayer()));
		updateGraphicDice();
		repaint();
		reroll.setEnabled(true);
		rollCount.setText("Rerolls left: "
				+ gamestate.getPlayersNumRerolls(gamestate.getCurrentPlayer()));
	}
